...
const AuthLinks = ({ isActive, isSearchExpanded ,location}) => {
    ...
    return (
        <div className={`flex space-x-4 ${isSearchExpanded && 'hidden sm:flex'}` }>
            <Link to="/signin" className={`px-3 py-2  text-base font-bold border border-blue-600  rounded hover:text-blue-500  text-blue-950   ${isActive === 'authSignin' ? 'bg-slate-100' : ''}`}>
                Sign In
            </Link>
